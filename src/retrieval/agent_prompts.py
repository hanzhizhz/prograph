...
FORCED_ANSWER_GENERATE_PROMPT = """你是一个专业的问答助手。虽然目前收集的信息可能不足以完全回答问题，但请根据已有的部分信息和你的内部知识，给出最可能的回答。
...
**要求：**
1. 结合已有证据和你的内部知识生成最可能的答案
2. 如果证据缺失，请明确指出基于内部知识推理的部分
3. 即使不确定，也要给出最可能的简短答案
4. 保持答案简洁明了
...
def get_forced_answer_prompt(
    ...
    Returns:
        Prompt 字符串
    """
    evidence_text = "\n".join(evidence) if evidence else "未收集到相关证据"

    return FORCED_ANSWER_GENERATE_PROMPT.format(
        question=question,
        evidence=evidence_text,
        history=history
    )
